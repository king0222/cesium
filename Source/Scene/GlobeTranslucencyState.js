 ...
 * @private
 */
GlobeTranslucencyState.prototype.update = function (globe, frameState) {
...
function isTranslucent(state) {
  return state._frontFaceTranslucencyMod !== TranslucencyMode.FRONT_OPAQUE;
}
...
function getNumberOfTextureUniforms(state, frameState) {
  ...
  if (requiresManualDepthTest(state, frameState)) {
...
function gatherDerivedCommandRequirements(state, frameState) {
  this._derivedCommandsLength = getDerivedCommandTypes(
    state,
    frameState,
    false,
    false,
    this._derivedCommandTypes
  );

  this._derivedBlendCommandsLength = getDerivedCommandTypes(
    state,
    frameState,
    true,
    false,
    this._derivedBlendCommandTypes
  );

  this._derivedPickCommandsLength = getDerivedCommandTypes(
    state,
    frameState,
    false,
    true,
  ...
  for (i = 0; i < this._derivedCommandsLength; ++i) {
    derivedCommandKey |= 1 << this._derivedCommandTypes[i];
  }
  for (i = 0; i < this._derivedBlendCommandsLength; ++i) {
    derivedCommandKey |= 1 << this._derivedBlendCommandTypes[i];
  }
  for (i = 0; i < this._derivedPickCommandsLength; ++i) {
    derivedCommandKey |= 1 << this._derivedPickCommandTypes[i];
  }

  var derivedCommandsToUpdateLength = 0;
  for (i = 0; i < derivedCommandsMaximumLength; ++i) {
    if ((derivedCommandKey & (1 << i)) > 0) {
      this._derivedCommandTypesToUpdate[derivedCommandsToUpdateLength++] = i;
    }
  }
  this._derivedCommandsToUpdateLength = derivedCommandsToUpdateLength;
...
function getDerivedCommandTypes(
  ...
  var manualDepthTest = requiresManualDepthTest(state, frameState);
  ...
  if (
    ...
    if (cameraUnderground) {
      ...
      types[length++] = DerivedCommandType.OPAQUE_FRONT_FACE_COMMAND;
      types[length++] = translucentBackFaceCommandType;
    } else {
      if (!isBlendCommand) {
        types[length++] = DerivedCommandType.DEPTH_ONLY_FRONT_FACE;
      }
      types[length++] = DerivedCommandType.OPAQUE_BACK_FACE_COMMAND;
      types[length++] = translucentFrontFaceCommandType;
    }
  } else if (
    translucencyMode ===
...
function getDerivedUniformMap(
  ...
  if (uniformMapChanged) {
    return combine(
      uniformMap,
      getDerivedUniformMapFunction(globeTranslucency),
      false
    );
  }
...
function createDerivedCommandPacks() {
  return [
    ...
    new DerivedCommandPack({
      ...
      getRenderStateFunction: getFrontFaceRenderState,
    ...
    new DerivedCommandPack({
      ...
      getRenderStateFunction: getBackFaceRenderState,
      getUniformMapFunction: undefined,
    }),
    // depthOnlyFrontFaceCommand
    new DerivedCommandPack({
...
GlobeTranslucencyState.executeGlobeCommands = function (
  frustumCommands,
  executeCommandFunction,
  scene,
  passState,
...
GlobeTranslucencyState.prototype.executeGlobeClassificationCommands = function (
  ...
  var frontFaceAlphaMode = this._frontFaceAlphaMode;
  var backFaceAlphaMode = this._backFaceAlphaMode;
  ...
  var frontTranslucent =
    frontFaceAlphaMode === TranslucencyMode.FRONT_TRANSLUCENT;
  ...
  globeTranslucencyFramebuffer.clearClassification();
  ...
  if (context.depthTexture) {
    ...
    var packedDepthTexture = this._packedDepthCommand.execute(
      context,
      passState
    );
